...
export default function DashboardPage() {
  ...
  const stats = [
    { label: t.dashboard.predictionsUsed, value: used, icon: "🎯", accent: "from-green-500 to-emerald-500" },
    { label: t.dashboard.yourPlan, value: user.plan?.toUpperCase() ?? "FREE", icon: "💎", accent: "from-blue-500 to-cyan-500" },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right px-4 py-2 rounded-xl bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20">
          ...
          <p className="text-[10px] text-gray-500 font-medium">{t.predictor.predictionsUsed}</p>
        </div>
      </div>
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-green-600/10 via-emerald-600/5 to-[#111118] border border-green-500/10 p-6">
        ...
        <div className="relative">
          <h3 className="text-lg font-bold text-white mb-1">{t.predictor.readyToAnalyze}</h3>
          <p className="text-gray-500 text-sm mb-4">{t.predictor.selectTeams}</p>
          <Link
            to="/predictor"
      ...
      <div className="bg-[#111118] border border-white/[0.06] rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-bold text-white">{t.dashboard.recentPredictions}</h2>
          <Link to="/history" className="text-xs text-green-400 hover:text-green-300 font-medium transition-colors">{t.dashboard.viewAll} →</Link>
        </div>
        {isLoading ? (
          <DashboardSkeleton />
          ...
          <div className="text-center py-10">
            <p className="text-gray-600 text-sm">{t.dashboard.noPredictions}</p>
            <Link to="/predictor" className="text-green-400 text-sm font-medium hover:text-green-300 mt-1 inline-block">{t.dashboard.getStarted} →</Link>
          </div>
        )}
      </div>
    </div>
