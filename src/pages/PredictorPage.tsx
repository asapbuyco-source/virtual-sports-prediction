      ...
      {team.recentForm && team.recentForm.length > 0 && (
        <div className="flex gap-1">
          {team.recentForm.slice(0, 5).map((f: string, i: number) => (
            <span key={i} className={`w-5 h-5 rounded flex items-center justify-center text-xs font-bold ${
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        {form.length > 0 && <button onClick={() => setForm([])} className="px-4 py-2 rounded text-xs text-gray-400 border border-gray-600 hover:bg-gray-700">Clear</button>}
      </div>
      <div className="flex flex-wrap gap-2">
        {form.map((f, i) => (
          <div key={i} className={`flex items-center gap-1 rounded px-2 py-1 text-xs border ${
            ...
            <button onClick={() => removeEntry(i)} className="text-gray-400 hover:text-red-400 ml-1">×</button>
          </div>
        ))}
      </div>
    </div>
      ...
      },
      {
        ...
        onError: (err: Error) => {
          if (err.message.includes("limit")) {
            toast.error("Prediction limit reached.", {
              duration: 5000,
              action: {
                label: "Upgrade",
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400">Last updated: {lastUpdated ? lastUpdated.toLocaleTimeString() : 'Never'}</p>
            </div>
            <button onClick={refresh} disabled={syncing} className="p-2 rounded-lg bg-green-600/20 text-green-400 hover:bg-green-600/30 transition disabled:opacity-50">
          ...
          <div className="bg-surface-1 border border-white/[0.06] rounded-2xl p-4 space-y-3 relative z-[60]">
             ...
             <div className="space-y-2 max-h-[200px] overflow-y-auto pr-1 custom-scrollbar">
                {DataService.getLiveMatches().map((m: any, i: number) => (
                  <button key={i} onClick={() => {
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-gray-400">Odds: {m.odds.home} | {m.odds.draw} | {m.odds.away}</p>
                    </div>
                ...
                {DataService.getLiveMatches().length === 0 && (
                  <p className="text-xs text-gray-400 text-center py-4 italic">No live matches synced. Run 'node scripts/fetch_live_odds.js' to sync.</p>
                )}
             </div>
          </div>
...
<select value={predictor.homeTeamId} onChange={e => { updatePredictor({ homeTeamId: e.target.value }); setResult(null); }}
                 className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-green-500/50 [&>option]:bg-surface-5 [&>option]:text-white">
...
<select value={predictor.awayTeamId} onChange={e => { updatePredictor({ awayTeamId: e.target.value }); setResult(null); }}
          ...
          {h2h && h2h.totalMatches >= 3 && (
            <div className="bg-surface-1 border border-green-500/20 rounded-2xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[{ label: "Home (1)", key: "oddsHome" }, { label: "Draw (X)", key: "oddsX" }, { label: "Away (2)", key: "oddsAway" }].map(o => (
                <div key={o.key} className="space-y-1">
                  <label className="text-xs text-gray-400">{o.label}</label>
                ...
                <div className="flex items-center justify-between flex-wrap gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-center px-4">
                      ...
                      <div className="text-xs text-gray-400">xG {result.expectedGoals}</div>
                    </div>
                    <div className="text-center">
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  {result.recommendedStake && (
                    <div className="flex flex-col items-end">
                      <span className="text-xs text-gray-400 uppercase">Suggested Stake</span>
                ...
                <div className="flex border-b border-white/[0.06]">
                  {(["tips", "signals", "stats"] as const).map((tab) => (
                    <button key={tab} onClick={() => setActiveTab(tab)}
                      className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest transition ${activeTab === tab ? "bg-white/[0.04] text-green-400 border-b-2 border-green-500" : "text-gray-400 hover:text-gray-300"}`}>
...
{result.tips.map((tip, i) => (
                         <div key={i} className={`rounded-xl p-4 border ${tip.tag === "SAFE" ? "bg-green-500/5 border-green-500/20" : tip.tag === "VALUE" ? "bg-yellow-500/5 border-yellow-500/20" : "bg-red-500/5 border-red-500/20"}`}>
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-xs text-gray-400 uppercase">{tip.market}</p>
...
{result.formAnalysis && (
                         <div className="bg-white/[0.03] border border-white/[0.06] rounded-lg p-3">
                          <p className="text-xs text-gray-400 uppercase mb-1">Form Summary</p>
